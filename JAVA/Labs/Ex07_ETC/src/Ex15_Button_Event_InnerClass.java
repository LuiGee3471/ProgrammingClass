...
class LoginForm3 extends Frame {
  ...
  public LoginForm3(String title) {
    ...
    // 1. 다른 클래스의 자원 사용(생성자 주소값 받기) ....
    // 2. Inner 클래스 사용해서 Outer 클래스가 가지는 ID, PWD값 함수에 사용
    //    클래스를 만들어서 사용
    // 3. 클래스까지 만들 필요가 있나요?
    // 3.1 재사용성이 중요하지 않나요?
    // 3.2 어차피 지금의 코드는 버튼 클릭 때만 사용되는데?
    
    // 최종 - 1회성 - 재사용성이 없다 - 코드의 간소화
    // 익명 타입 (실제로 인터페이스는 직접 개체 생성이 불가능)
    // 인터페이스를 클래스 없이도 직접 구현하는 방법 제공
