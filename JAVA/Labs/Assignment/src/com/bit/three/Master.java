...
//			showStatus();
			...
			System.out.println("9. 자판기 전원 Off \n");
			...
			case "9" : Login.progressBar("모든 프로그램을 종료합니다.", 1000);System.exit(1);; break; // 로그인 화면으로 돌아가기
			//case "0" : showMemberInfo(); break; 
	...
	private void showMemberInfo() {
		System.out.println("\n------------------------------\n");
		Login.progressBar("DB 서버와 통신 중");
		System.out.println("[Status_Code : 200]");
		System.out.println("\n------------------------------\n");
		System.out.println("		[증명서 자판기 회원 DB서버]\n");
		System.out.println("(인원) 학번   - (이름 / 생년월일 / 학년 / 전공학과)\n");
		for(int i=0; i<Student.index; i++) { //2018(정규현,920812,3,컴퓨터전공)
			System.out.println( "("+(i+1)+") "+Student.studentRegisterArr[i].getStudentNumber() +" - ("+Student.studentRegisterArr[i].getName()
					+" / "+ Student.studentRegisterArr[i].getResidentNumber()+" / "+ Student.studentRegisterArr[i].getGrade()+" / "
					+Student.studentRegisterArr[i].getMajor()+")");
		}
//		System.out.println("\n------------------------------\n");
	}
