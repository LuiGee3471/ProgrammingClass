...
public class boarddao {
  ...
  public List<board> list(int cpage, int pagesize) {
     ...
     *       content, writedate, readnum , filename, filesize , refer , depth , step 
     *       from (SELECT * 
     *             FROM jspboard 
     *             ORDER BY refer DESC , step ASC) 
     *       where rownum <= 6 --endrow ) 
     * where rn >= 4; --firstrow
     */
  ...
  public board getContent(int idx) {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        boarddto = new board(idx, writer, pwd, subject, content, writedate, readnum, filename, filesize, homepage,
